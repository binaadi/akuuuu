...
const looksMp4 = u => /\.mp4(\?.*)?$/i.test(u);

function resolveFinal(video) {
  if (isHttp(video.video_id)) {
    if (looksHls(video.video_id)) return { url: video.video_id, type: "hls" };
    if (looksMp4(video.video_id)) return { url: video.video_id, type: "mp4" };
    return { url: video.video_id, type: "iframe" };
  }
  if (video.source === "videy") {
    return { url: `https://cdn.videy.co/${encodeURIComponent(video.video_id)}.mp4`, type: "mp4" };
  }
  if (video.source?.includes("dood") || video.source === "doodstream") {
    return { url: `https://dsvplay.com/e/${encodeURIComponent(video.video_id)}`, type: "iframe" };
  }
  if (video.source?.includes("videq")) {
    return { url: `https://videq.pw/e/${encodeURIComponent(video.video_id)}`, type: "iframe" };
  }
  if (video.source?.includes("lixstream")) {
    return { url: `https://lixstream.com/e/${encodeURIComponent(video.video_id)}`, type: "iframe" };
  }
  return {
    url: video.video_id,
    type: looksHls(video.video_id) ? "hls" : (looksMp4(video.video_id) ? "mp4" : "iframe")
  };
}
...
/**
 * 🔗 Ambil video publik by embed_token
 */
...
/**
 * 🔑 Generate signed streaming URL (PRIVATE – butuh login uploader)
 */
router.get("/:id/signed", authenticateToken, (req, res) => {
  db.get("SELECT id, video_id, source FROM videos WHERE id=? AND user_id=?", 
    ...
    (err, video) => {
      ...
      const { type } = resolveFinal(video);
      res.json({ url: `/api/videos/${video.id}/stream?token=${token}`, type });
    }
  );
});

/**
 * 🔑 Generate signed streaming URL (PUBLIC – via embed_token)
 */
router.get("/public-signed/:token", (req, res) => {
  db.get("SELECT id, video_id, source FROM videos WHERE embed_token = ?", 
    [req.params.token], 
    (err, video) => {
      if (err) return res.status(500).json({ error: "DB error" });
      if (!video) return res.status(404).json({ error: "Video tidak ditemukan" });
